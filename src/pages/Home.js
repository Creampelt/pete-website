...
export default class Home extends React.Component {
  ...
  calcScrollPos = () => {
    if (this.meetRef.current.getBoundingClientRect().top <= NAVBAR_HEIGHT) {
      this.props.setColors(["#EEF4F8", "#EEF4F8", "#F2BA42"]);
    } else if (this.aboutRef.current.getBoundingClientRect().top <= NAVBAR_HEIGHT) {
      this.props.setColors(["#EEF4F8", "#F2BA42", "#EEF4F8"]);
    } else {
      this.props.setColors(["#F2BA42", "#EEF4F8", "#EEF4F8"]);
    }
  };
  ...
  render() {
    return (
      <div className={"App"}>
        <div className={"row river-blue"} id={"home"}>
